      ...
      <article className="max-w-3xl mx-auto bg-white dark:bg-slate-800 shadow-xl rounded-lg p-5 sm:p-8 lg:p-10">
          ...
          {categoriesData && categoriesData.length > 0 && (
            <div className="flex flex-wrap gap-2">
              {categoriesData.map(category => (
                category.slug && (
                  <Link key={category.id} href={`/categories/${category.slug}`} className="bg-slate-100 dark:bg-slate-700 text-slate-700 dark:text-slate-300 px-3 py-1 rounded-full hover:bg-slate-200 dark:hover:bg-slate-600 text-xs font-medium">
                    {category.name}
                  </Link>
                )
              ))}
        ...
        {fullCoverImageUrl && (
          <div className="relative mb-8 w-full overflow-hidden rounded-lg shadow-md aspect-video">
            <Image src={fullCoverImageUrl} alt={article.cover_image?.alternativeText || article.title || 'Article cover image'} className="object-cover" fill sizes="(max-width: 896px) 90vw, 896px" priority />
          </div>
        )}
        {article.ai_assisted && (
