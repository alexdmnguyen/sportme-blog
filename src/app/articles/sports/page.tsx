...
export default async function SportsArticlesPage(props: { searchParams?: { page?: string } }) {
  ...
  return (
    <main className="container mx-auto px-4 py-8 sm:px-6 lg:py-12">
      <h1 className="text-4xl sm:text-5xl font-extrabold mb-10 sm:mb-14 text-center text-slate-900 dark:text-slate-100">All Sports Articles</h1>
      ...
      {articles.length > 0 ? (
        <>
          ...
          {pagination && pagination.pageCount > 1 && (
            <div className="mt-12 flex flex-col sm:flex-row justify-center items-center gap-4">
              {currentPage > 1 && (
                <Link href={`/articles/sports?page=${currentPage - 1}`} className="w-full sm:w-auto text-center px-4 py-2 bg-indigo-600 text-white rounded-md hover:bg-indigo-700 transition-colors">
                  &larr; Previous
                </Link>
              )}
        ...
        <p className="text-center text-slate-600 dark:text-slate-400 text-xl py-10">No sports articles found. Check back later!</p>
      )}
    </main>
  );
}